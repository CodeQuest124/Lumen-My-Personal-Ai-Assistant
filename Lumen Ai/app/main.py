...
async def lifespan(app: FastAPI):
    ...
    logger.info("[CONFIG] Tavily API key: %s","configure" if TAVILY_API_KEY else "NOT SET")
    ...
    try:
        ...
        logger.info("Realtime Groq initialized successfully")
        
        logger.info("Initializing chat service...")
        chat_service = ChatService(groq_service, realtime_service)
        logger.info("Chat service initilized successfully")
        ...
        logger.info("=" * 60)
        logger.info("L.U.M.E.N is online and ready")
...
app = FastAPI(
    ...
    description="Just A Rather Very Intellegent System",
    lifespan=lifespan,
    docs_url=None,
    redoc_url=None,
    openapi_url=None
...
class TimineMiddleware(BaseHTTPMiddleware):
...
app.add_middleware(TimineMiddleware)
#=============================================================
# API ENDPOINTS
#=============================================================
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        logger.error("[API /chat] Rate limit hit: %s", e)
...
def _split_sentences(buf: str):
    ...
    raw = [p.strip() for p in parts[:-1] if p.strips()]
    sentences, pending = [], ""
...
async def chat_realtime(request: ChatRequest):
    ...
    try:
        ...
        chat_service.save_chat_session(session_id)
        logger.info("[API /chat/realtime] Done | session-id=%s | response_len=%d", session_id[:12], len(response_text))
        return ChatResponse(response=response_text, session_id=session_id)
        ...
        logger.warning("[API /chat/realtime] All Groq APIs failed: %s", e)
        raise HTTPException(status_code=503, detail=str(e))
...
async def chat_realtime_stream(request: ChatRequest):
    ...
    if not chat_service or not realtime_service:
        raise HTTPException(status_code=503, detail="Service not initiated")
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        logger.error("[API /chat] All APIs failed: %s", e)
        raise HTTPException(status_code=503, detail=str(e))
...
def _merge_short(sentences):
    ...
    while i < len(sentences):
        cur = sentences[i]
        j = i + 1
        # FIX: was concatenating with cur twice; now correctly merges short follow-on sentences
        while j < len(sentences) and len(sentences[j].split()) <= _MERGE_IF_WORDS:
            cur = (cur + " " + sentences[j]).strip()
            j += 1
        merged.append(cur)
        ...
        for chunk in chunk_iter:
            ...
            for i, sent in enumerate(sentences):
                ...
                if is_last:
                    ...
                    _submit(sent)
                    is_first = False  # FIX: was typo `is_firt`

    except Exception as e:
        # FIX: was `for fut, in audio_queue` (trailing comma unpacking bug)
...
# FIX: was "/chat/ realtime/stream" (space in URL)
@app.post("/chat/realtime/stream")
async def chat_realtime_stream(request: ChatRequest):
    if not chat_service or not realtime_service:
...
@app.get("/chat/jarvis/stream")
async def chat_jarvis_stream(request: ChatRequest):
    if not chat_service:
        raise HTTPException(status_code=503, detail="Service not initiated")
    logger.error("[API /chat/jarvis/stream] Incoming | seddion_id=%s | message_len=%d | message=%.100s",
                 request.session_id or "new", len(request.message), request.message)
    try:
        session_id = chat_service.get_or_create_session(request.session_id)
