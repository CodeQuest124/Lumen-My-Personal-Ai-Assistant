...
GROQ_REQUESTS_TIMEOUT_FAST = 15  # seconds

_QUERY_EXTRACTION_PROMT = (
    ...
    "information the user needs. Resolve any references (like 'that website', 'him', 'its') "
    "using the conversation history. Output ONLY th search query, nothing else."
)
...
class RealtimeGroqService(GroqService):
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        if tavily_api_key:
            ...
            logger.warning("TAVILY_API_KEY not set. Real-time search will be unavailable.")
        ...
        if GROQ_API_KEYS:
            ...
            self._fast_llm = ChatGroq(
                ...
                temperature=0.0,
                request_timeout=GROQ_REQUESTS_TIMEOUT_FAST,
                max_tokens=50,
            )
        else:
            self._fast_llm = None
    ...
    def _extract_search_query(
        ...
        try:
            ...
            if extracted and 3 <= len(extracted.split()) <= 200:
                logger.info(
                    "[REALTIME] Query extraction: '%s' -> '%s' (%.3fs)",
                    question[:80], extracted[:80], time.perf_counter() - t0,
                )
                return extracted
            logger.warning("[REALTIME] Query extraction unstable result, using raw question")
            return question
    ...
    def search_tavily(self, query: str, num_results: int =7) -> str:
        ...
        try:
            ...
            if not results and not ai_answer:
                logger.warning(f"No Tavily search results found for query: %s", query)
                return ("", None)
            ...
            if results:
                parts.append("INDIVOIDUAL SOURCES:")
    ...
    def get_response(self, question: str, chat_history: Optional[List[tuple]] = None) -> str:
        ...
        try:
            ...
            logger.info(
                "[REALTIME] Realtime chat | Length: %d chars | Preview: %.120s",
                len(response_context), response_context,
            )
...
class RealtimeGroqService(GroqService):
    ...
    def search_tavily(self, query: str, num_results: int = 7) -> Tuple[str, Optional[dict]]:
        ...
        try:
            ...
            # FIX: was mixing up 'r' and 'result' variable names in the list comprehension
            payload: Optional[dict] = {
                "query": query,
                "answer": ai_answer,
                "results": [
    ...
    def get_response(self, question: str, chat_history: Optional[List[tuple]] = None) -> str:
        try:
            ...
            t0 = time.perf_counter()
            response_text = self._invoke_llm(prompt, messages, question)
            logger.info("[TIMING] groq_api: %.3fs", time.perf_counter() - t0)
            logger.info(
                "[REALTIME] Realtime chat | Length: %d chars | Preview: %.120s",
