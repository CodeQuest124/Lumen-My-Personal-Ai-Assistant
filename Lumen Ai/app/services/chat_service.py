...
from app.services.brain_service import BrainServce
...
class ChatService:
    ...
    def __init__(
        ...
        brain_service: BrainServce = None,
    ):
        self.groq_service = groq_service
        self.realtime_service = realtime_service
        self.brain_service = brain_service
        self.sessions: Dict[str, List[ChatHistory]] = {}
        self._save_lock = threading.Lock()
        
#=============================================================
#   SESSION LOAD / VALIDATE / GER-OR-CREATE
#=============================================================
    ...
    def load_session_from_disk(self, session_id: str) -> bool:
        ...
        try:
            ...
            messages = []
            for msg in chat_dict.get("message", []):
    ...
    def get_or_create_session(self, session_id: Optional[str] = None) -> str:
        ...
        if not self.validate_session_id(session_id):
            raise ValueError(
                ...
                "not contain path traversal characters, and be less than 255 characters."
            )
    ...
    def process_message(self, session_id: str, user_message: str) -> str:
        ...
        self.add_message(session_id, "user", content=user_message)
    ...
    def process_jarvis_message_stream(
        ...
        def _run_brain():
            if self.brain_service and brain_idx is not None:
                ...
                return (qt, r, ms)
            return("realtime", "No braib service", 0)
        ...
        with ThreadPoolExecutor(max_workers=2) as executer:
            future_brain = executer.submit(_run_brain)
            future_search = executer.submit(_run_search)
            try:
            ...
            if query_type == "general":
                ...
                try:
                    ...
                    logger.warning("[JARVIS] Wed search prefetch timed out after %ds", JARVIS_BRAIN_SEARCH_TIMEOUT)
                    formatted_results, search_payload = "", None
        ...
        try:
            if query_type == "general":
                ...
                if not self.realtime_service:
                    raise ValueError("Realtime service nit initalized.")
                stream = self.realtime_service.stream_response_with__prefetched(
            ...
            for chunk in stream:
                ...
                self.sessions[session_id][-1].content += chunk
                chunck_count +=1
                if chunck_count % SAVE_EVERY_N_CHUNKS == 0:
                    self.save_chat_session(session_id, log_timing=False)
                yield chunk
        finally:
            final_response = self.sessions[session_id][-1].content
            logger.info("[JARVIS-STREAM] Completed | Route: %s | Chunks: %d | Response length: %d chars",
                        query_type, chunck_count, len(final_response))
            self.save_chat_session(session_id)
