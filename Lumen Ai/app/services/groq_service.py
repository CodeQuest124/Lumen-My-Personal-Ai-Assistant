...
ALL_APIS_FAILED_MESSAGE = (
    "I'm unable to processmyour request at the moment. All API services are"
    "tempoparily unavailable. Please try again in a few minutes."
)
...
class GroqService:
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        if not GROQ_API_KEYS:
            raise ValueError(
                "No GROQ API keys configured. Please set the GROQ_API_KEY (and optionally GROQ_API_KEY_2, GROQ_API_KEY_3, ...) in .env"
        ...
        for i in range(n):
            ...
            for i in range(n):
                ...
                try:
                    ...
                    if i > 0:
                        logger.info("Fallback successful: API key #{i + 1}/{n} succeeded: {masked_key}")
                    ...
                    if _is_rate_limit_error(e):
                        ...
                        logger.error(f"API key #{i + 1}/{n} failed:{masked_key} - {str(e)[:100]}")
        ...
        def _stream_llm(
            ...
            for i in range(n):
                ...
                try:
                    ...
                    for chunk in chain.stream_invoke({"history": messages, "question": question}):
                        content = ""
            ...
            def _build_prompt_and_messages(
                ...
                extra_system_parts: Optional[str] = None,
                mode_addendum: str = "",
            ) -> tuple:
                ...
                try:
                    ...
                    if context_docs:
                        ...
                        logger.info("[CONTEXT] Retrived %d chuks from sources: %s", len(context_docs), context_sources)
                    else:
                ...
                logger.info("[PROMPT] System message lenghth: %d chars | History pairs: %d | question: %.100s",
            ...
            def get_response(
                ...
                try:
                    ...
                    raise Exception(f"Error in getting response from Groq: {str(e)}") from e
            ...
            def stream_response(
                ...
                try:
                    ...
                    raise Exception(f"Error in streaming response from Groq: {str(e)}") from e                 
...
class GroqService:
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        if not GROQ_API_KEYS:
            raise ValueError(
                "No GROQ API keys configured. Please Set GROQ_API_KEY (and optionally GROQ_API_KEY_2, GROQ_API_KEY_3, ...) in .env"
            )
        ...
        try:
            ...
            if context_docs:
                ...
                logger.info(
                    ...
                    len(context_docs), context_sources,
                )
            else:
                logger.info("[CONTEXT] No relevant chunks found for query.")
        except Exception as retrieval_err:
    ...
    def stream_response(
        ...
        try:
            ...
            yield {"_activity": {"event": "context_retried", "message": "Retried relevent context from knowledge base"}}
...
class GroqService:
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        if not GROQ_API_KEYS:
            raise ValueError(
                "No GROQ API keys configured. Please set GROQ_API_KEY "
    ...
    def _invoke_llm(
        ...
        masked_all = ", ".join([_mask_api_key(GROQ_API_KEYS[k]) for k in keys_tried])
        logger.error(f"All {n} API key(s) failed. Tried: {masked_all}")
        raise AllGroqApisFailedError(ALL_APIS_FAILED_MESSAGE) from last_exc
    ...
    def _stream_llm(
        ...
        for j in range(n):          # FIX: was `for i in range(n)` — j is needed below
            i = (key_start_index + j) % n   # j is now defined
            masked_key = _mask_api_key(GROQ_API_KEYS[i])
            logger.info(f"Streaming with API key #{i + 1}/{n}: {masked_key}")
