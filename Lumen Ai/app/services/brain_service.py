...
- realtime: ALWAYS use realtime for:
    ...
    ● Anything that could have changed: news, weather, stock prices, sports score, elections, "latest", "current", "today", "recent", "now".
    ● Factual lookups where real-time data would be better: events, companies, product, releases, versions.
    ...
    def __init__(self):
        ...
        if GROQ_API_KEYS:
            try:
                ...
                logger.info("[BRAIN] Groq brain initialized (model: %s) with %d key(s)", GROQ_BRAIN_MODEL, len(self.llms))
            except Exception as e:
                logger.warning("[BRAIN] Failed to create Groq brain: %s", e)
        if not self._llms:
                logger.warning("[BRAIIN] No API keys. Classification will default to realtime.")
    ...
    def classify(
        ...
        if chat_history:
            for u, a in chat_history[-MAX_CONTEXT_TURNS:]:
                u_preview = (u or "")[:MAX_MESSAGE_PREVIEW] + ("_" if len(u or "") > MAX_MESSAGE_PREVIEW else "")
                a_preview = (a or "")[:MAX_MESSAGE_PREVIEW] + ("_" if len(a or "") > MAX_MESSAGE_PREVIEW else "")
                context_lines.append(f"User: {u_preview}")
                context_lines.append(f"Assistant: {u_preview}")
        context_block = "\n".join(context_lines) if context_lines else "(No prior conversations)"
...
Classify the current message. Output ONLY: general or realtime"""
        ...
        try:
            ...
            return ("realtime", f"API eoor: {str(e)[:60]}", elapsed_ms)
        
        elapsed_ms = int((time.perf_counter() - t0) * 1000)
        if re.search(r"\brealtime\b", text):
            logger.info("[BRAIN] Groq (key #%d) returned related in %d ms", key_index + 1, elapsed_ms)
            return ("realtime", REASONING_REALTIME, elapsed_ms)
        if re.search(r"\bgeneral\b", text):
            logger.info("[BRAIN] Groq (key #%d) returned related in %d ms", key_index + 1, elapsed_ms)
            return ("general", REASONING_REALTIME, elapsed_ms)
