    ...
    def load_learning_data(self) -> List[Document]:
        ...
        for file_path in sorted(LEARNING_DATA_DIR.glob("*.txt")):
            try:
                ...
                logger.warning(f"Could not load learning data file %s: %s", file_path, e)
    ...
    def load_chat_history(self) -> List[Document]:
        ...
        for file_path in sorted(CHATS_DATA_DIR.glob("*.json")):
            try:
                ...
                logger.warning(f"Could not load chat history file %s: %s", file_path, e)
    ...
    def save_vector_store(self):
        if self.vector_store:
            try:
                ...
                logger.error(f"Failed to save vector store to disk: %s", e)
